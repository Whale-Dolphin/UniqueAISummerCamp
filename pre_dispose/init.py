...
def knn_impute(df, idx, k):
    ...
    distances = non_missing.apply(lambda x: distance(x, df.loc[idx]), axis=1)
    nearest = non_missing.iloc[distances.argsort()[:k]]
    impute_value = nearest.mean()
    return impute_value

for col in df_copy_4.columns:
...
for col in df.columns:
    min_val = df[col].min()
    max_val = df[col].max()
    df_copy_5[col] = (df[col] - min_val) / (max_val - min_val)
...
df_copy_8 = df.copy()
for col in df.columns:
    df_copy_8[col] = 1 / (1 + np.exp(-df_copy_8[col]))
